...
test('test', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Other questions you may have' })).toBeVisible({ timeout: 30_000 });
  await expect(page.locator('h3')).toContainText('Other questions you may have');
  ...
  await page.locator('div').filter({ hasText: /^What is small non - obstructing calculus bladder\?$/ }).getByRole('img').click();
  await expect(page.getByLabel('Detailed Explanation')).toMatchAriaSnapshot(`
    - img
    - img
    - textbox "Feedback"
