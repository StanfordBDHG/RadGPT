...
describe("Unauthenticated", () => {
    ...
    beforeAll(async () => {
        ...
        testDoc = unauthenticatedDb.collection("users/user1/annotations").doc("testAnnotation");
    })
...
describe("Authenticated Processed Annotations Access", () => {
    ...
    beforeEach(async () => {
        ...
        await db.withSecurityRulesDisabled(async (context) => {
            await setDoc(doc(context.firestore(), "users/user1/annotations/testAnnotations"), fileContent)
        })

        return async () => {
            await db.withSecurityRulesDisabled(async (context) => {
                await deleteDoc(doc(context.firestore(), "users/user1/annotations/testAnnotations"))
            })
        }
    })
...
describe("Authenticated Cached Answer Access", () => {
    ...
    beforeEach(async () => {
        ...
        user2 = db.authenticatedContext("user2").firestore();
        await db.withSecurityRulesDisabled(async (context) => {
            await setDoc(doc(context.firestore(), "users/user1/annotations/cached_answer_1"), fileContent)
        })

        return async () => {
            await db.withSecurityRulesDisabled(async (context) => {
                await deleteDoc(doc(context.firestore(), "users/user1/annotations/cached_answer_1"))
            })
        }
    })
    test("Test Allowed User1 Read Access", async () => {
    ...
    test("Test File Update", async () => {
        ...
        await assertFails(
            updateDoc(doc(user1, "users/user1/annotations/cached_answer_1"), {
                ...fileContent,
                feedback: {},
                main_explanation: "Different Main Explanation",
            })
        )
        await assertFails(
            updateDoc(doc(user1, "users/user1/annotations/cached_answer_1"), {
                ...fileContent,
                feedback: {},
                concept_based_question: "Different Concept-Based Question",
            })
        )
        await assertFails(
            updateDoc(doc(user1, "users/user1/annotations/cached_answer_1"), {
                ...fileContent,
                feedback: {},
                concept_based_question_answer: "Different Concept-Based Question Answer",
            })
        )
        await assertFails(
            updateDoc(doc(user1, "users/user1/annotations/cached_answer_1"), {
                ...fileContent,
                feedback: {},
                concept_based_template_question: "Different ConcepteBased Template Question",
            })
        )
        await assertFails(
            updateDoc(doc(user1, "users/user1/annotations/cached_answer_1"), {
                ...fileContent,
                feedback: {},
                concept_based_template_question_answer: "Different Concept-Based Template Question Answer",
            })
        )
    })
})