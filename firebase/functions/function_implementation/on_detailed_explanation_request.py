...
def __get_cached_answer(
    uid: int, file_name: str, observation_index: int
...
def __store_detailed_response(
    uid: int, file_name: str, observation_index: int, detailed_response: dict[str, Any]
...
def on_detailed_explanation_request_impl(req: https_fn.Request) -> https_fn.Response:
    ...
    if not (0 <= observation_index < len(processed_annotations)):
        raise https_fn.HttpsError(
            code=https_fn.FunctionsErrorCode.INVALID_ARGUMENT,
            message="Invalid observation id.",
        )
    processed_annotation = processed_annotations[observation_index]

    concept = __get_concept(processed_annotation)

    detailed_response = dataclasses.asdict(request_gpt(user_provided_text, concept))
    __store_detailed_response(uid, file_name, observation_index, detailed_response)
