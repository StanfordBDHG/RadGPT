...
def _get_end_ix_for_start_ix(radgraph_output: Dict[str, Any], start_idx: int) -> int:
    ...
    for processed_annotation in processed_annotations:
        ...
        for located_at_start_observation in located_at_start_observations:
            ...
            for start_idx in located_at_start_observation:
                end_indices.append(_get_end_ix_for_start_ix(radgraph_output, start_idx))
...
def _map_all_token_entities_to_text_ranges(
...
def _map_radgraph_relevant_token_entities_to_text_ranges(
    total_tokens: List[str],
    user_provided_text: str,
    radgraph_relevant_entities: List[Tuple[int, int]],
) -> Dict[int, Dict[str, int]]:
    total_entities_to_text_ranges_dict = _map_all_token_entities_to_text_ranges(
...
def _flat(list_of_list: List[List[T]]) -> List[T]:
    return [item for list in list_of_list for item in list]


def determine_entities_in_user_entered_text(
    ...
    relations = [
        ...
        for processed_annotation in processed_annotations
        for start_idx, end_idx in zip(
            _flat(processed_annotation["located_at_start_ix"]),
            _flat(processed_annotation["located_at_end_ix"]),
        )
    ]
    ...
    radgraph_relevant_entities_to_text_ranges_mapping = (
        _map_radgraph_relevant_token_entities_to_text_ranges(
